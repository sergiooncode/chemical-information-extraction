...
from src.application.service.process_document_app_service import process_document_app_service
...
class DistributeDocumentBatchesAppService(ApplicationService):
    ...
    def execute(self):
        ...
        unprocessed_document_ids = [ud.id() for ud in unprocessed_documents]

        document_id_batches = chunk_it(unprocessed_document_ids,
                                       NUMBER_OF_DOCUMENT_PROCESSING_WORKERS)

        jobs = []
        for batch in document_id_batches:
            p = multiprocessing.Process(target=process_document_app_service,
                                        args=(batch,))
            jobs.append(p)
            p.start()
